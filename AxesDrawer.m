        ...
        withAnchor:(int)anchor
{
	if ([text length])
	{
		...
		switch (anchor) {
			case ANCHOR_TOP: textRect.origin.y += textRect.size.height
                / 2 + VERTICAL_TEXT_MARGIN; break;
			case ANCHOR_LEFT: textRect.origin.x += textRect.size.width
                / 2+ HORIZONTAL_TEXT_MARGIN; break;
			case ANCHOR_BOTTOM: textRect.origin.y -= textRect.size.height
                / 2 + VERTICAL_TEXT_MARGIN; break;
			case ANCHOR_RIGHT: textRect.origin.x -= textRect.size.width
                / 2+ HORIZONTAL_TEXT_MARGIN; break;
		}
                      ...
                      scale:(CGFloat)pointsPerUnit
{
	...
	if (boundsContainsOrigin) {
		if ((axisOrigin.x - pixelsPerHashmark < bounds.origin.x) &&
			(axisOrigin.x + pixelsPerHashmark >
             bounds.origin.x + bounds.size.width) &&
			(axisOrigin.y - pixelsPerHashmark < bounds.origin.y) &&
			(axisOrigin.y + pixelsPerHashmark >
	...
	for (int offset = unitsPerHashmark; !started ||
         stillGoing; offset += unitsPerHashmark)
	{
		BOOL drew = NO;
		...
		if (CGRectContainsPoint(bounds, hashMarkPoint)) {
			CGContextMoveToPoint(context, hashMarkPoint.x,
                                 hashMarkPoint.y-HASH_MARK_SIZE);
			CGContextAddLineToPoint(context, hashMarkPoint.x,
                                    hashMarkPoint.y+HASH_MARK_SIZE);
			[self drawString:[NSString stringWithFormat:@"%d", offset]
                     atPoint:hashMarkPoint withAnchor:ANCHOR_TOP];
			drew = YES;
		}
		hashMarkPoint.x = axisOrigin.x-scaledOffset;
		if (CGRectContainsPoint(bounds, hashMarkPoint)) {
		...
		if (CGRectContainsPoint(bounds, hashMarkPoint)) {
			...
			CGContextAddLineToPoint(context, hashMarkPoint.x+HASH_MARK_SIZE,
                                    hashMarkPoint.y);
			[self drawString:[NSString stringWithFormat:@"%d", offset]
                     atPoint:hashMarkPoint withAnchor:ANCHOR_LEFT];
			drew = YES;
		}
		hashMarkPoint.y = axisOrigin.y+scaledOffset;
		if (CGRectContainsPoint(bounds, hashMarkPoint)) {
